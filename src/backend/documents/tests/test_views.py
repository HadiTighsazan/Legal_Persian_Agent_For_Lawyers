...
# ---------------------------------------------------------------------------
# Tests — DocumentEmbedView (POST /documents/<uuid>/embed/)
# ---------------------------------------------------------------------------
...
class DocumentEmbedViewTests(TestCase):
    ...
    def test_successful_embed_returns_202(self, mock_embed: MagicMock) -> None:
    ...
    def test_embed_counts_unembedded_chunks(self, mock_embed: MagicMock) -> None:
        ...
        # Create some chunks without embeddings.
        for i in range(3):
            DocumentChunk.objects.create(
                document=self.document,
                chunk_index=i,
    ...
    def test_embed_skips_already_embedded_chunks(self, mock_embed: MagicMock) -> None:
        ...
        # Create a chunk with an embedding (must be 1536 dimensions for pgvector).
        DocumentChunk.objects.create(
            document=self.document,
            chunk_index=0,
            page_start=1,
        ...
        data = response.json()
        # Only the un-embedded chunk should be counted.
        self.assertEqual(data["total_chunks"], 1)


# ---------------------------------------------------------------------------
# Tests — ChunkBatchEmbedView (POST /chunks/batch-embed/)
# ---------------------------------------------------------------------------
...
class ChunkBatchEmbedViewTests(TestCase):
    ...
    def test_invalid_chunk_ids_returns_400(self) -> None:
...
# ---------------------------------------------------------------------------
# Tests — ChunkReEmbedView (POST /chunks/<uuid>/re-embed/)
# ---------------------------------------------------------------------------
...
class ChunkReEmbedViewTests(TestCase):
    ...
    def test_other_users_chunk_returns_403(self) -> None:
...
# ---------------------------------------------------------------------------
# Tests — TaskStatusView (GET /tasks/<uuid>/)
# ---------------------------------------------------------------------------
...
class TaskStatusViewTests(TestCase):
    ...
    def test_returns_task_status(self) -> None:
