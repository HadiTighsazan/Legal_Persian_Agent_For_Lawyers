...
| **mode** | **VARCHAR(20)** | **DEFAULT 'global_rag', NULL, INDEXED** | **Conversation type: local_rag, global_rag, strategist, action_engine. Null defaults to global_rag for backward compatibility. (added in migration 0004 of conversations app — Phase 3)** |
| created_at | TIMESTAMP | DEFAULT NOW() | Creation timestamp |
...
- **`idx_conversations_mode`** on **`mode`** (added in migration 0004 — Phase 3)
...
| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| id | UUID | PRIMARY KEY | Case profile unique identifier |
| conversation_id | UUID | FOREIGN KEY (conversations.id) ON DELETE CASCADE, UNIQUE | Parent strategist conversation |
| case_type | VARCHAR(100) | NOT NULL | Case type: contract_dispute, family_law, criminal, etc. |
| facts | JSONB | DEFAULT '{}' | Structured facts: {parties, claims, evidence, timeline, ...} |
| completeness_score | FLOAT | DEFAULT 0.0 | Fact completeness score (0.0–1.0) |
| is_complete | BOOLEAN | DEFAULT FALSE | True when enough facts gathered for analysis |
| created_at | TIMESTAMP | DEFAULT NOW() | Creation timestamp |
| updated_at | TIMESTAMP | DEFAULT NOW() | Last update timestamp |

**Indexes:**
- `idx_case_profiles_conversation_id` on `conversation_id`
- `idx_case_profiles_case_type` on `case_type`

**Notes:**
- Created in migration 0004 of conversations app (Phase 3 — Interactive Strategist)
- One-to-one with Conversation (only strategist-mode conversations have a CaseProfile)
- `facts` JSONB is populated incrementally by the StrategistService as the LLM-driven interview progresses

---

### 10. strategic_reports
| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| id | UUID | PRIMARY KEY | Report unique identifier |
| conversation_id | UUID | FOREIGN KEY (conversations.id) ON DELETE CASCADE, UNIQUE | Parent strategist conversation |
| case_profile_id | UUID | FOREIGN KEY (case_profiles.id) ON DELETE CASCADE | Source case profile |
| success_probability | FLOAT | NOT NULL | Estimated success likelihood (0.0–1.0) |
| summary | TEXT | NOT NULL | Report summary |
| strengths | JSONB | DEFAULT '[]' | List of case strengths |
| weaknesses | JSONB | DEFAULT '[]' | List of case weaknesses |
| risks | JSONB | DEFAULT '[]' | List of identified risks |
| recommendations | JSONB | DEFAULT '[]' | List of recommendations |
| applicable_laws | JSONB | DEFAULT '[]' | Applicable laws: [{title, articles, citations}] |
| applicable_precedents | JSONB | DEFAULT '[]' | Applicable precedents: [{title, number, summary}] |
| raw_report | TEXT | NOT NULL | Full Persian markdown report |
| created_at | TIMESTAMP | DEFAULT NOW() | Creation timestamp |

**Indexes:**
- `idx_strategic_reports_conversation_id` on `conversation_id`
- `idx_strategic_reports_case_profile_id` on `case_profile_id`

**Notes:**
- Created in migration 0004 of conversations app (Phase 3 — Interactive Strategist)
- One-to-one with Conversation (one report per strategist session)
- Generated by the StrategistService after the interview phase completes
...
### Migration 0004 (conversations) — Add mode, CaseProfile, StrategicReport (Phase 3 — Interactive Strategist)
- **File:** `src/backend/conversations/migrations/0004_add_mode_caseprofile_strategicreport.py`
- **Changes:**
  - Added `mode` column to `conversations` table: `VARCHAR(20)`, default `'global_rag'`, nullable, indexed
  - Choices: `local_rag`, `global_rag`, `strategist`, `action_engine`
  - Created `idx_conversations_mode` index on `mode`
  - Created `case_profiles` table with columns: `id` (UUID PK), `conversation_id` (UUID FK, UNIQUE), `case_type` (VARCHAR(100)), `facts` (JSONB), `completeness_score` (FLOAT), `is_complete` (BOOLEAN), `created_at`, `updated_at`
  - Created `strategic_reports` table with columns: `id` (UUID PK), `conversation_id` (UUID FK, UNIQUE), `case_profile_id` (UUID FK), `success_probability` (FLOAT), `summary` (TEXT), `strengths` (JSONB), `weaknesses` (JSONB), `risks` (JSONB), `recommendations` (JSONB), `applicable_laws` (JSONB), `applicable_precedents` (JSONB), `raw_report` (TEXT), `created_at`
  - Created indexes on `case_profiles.conversation_id`, `case_profiles.case_type`, `strategic_reports.conversation_id`, `strategic_reports.case_profile_id`
- **Purpose:** Enables the Interactive Strategist mode by adding conversation mode routing, structured case fact storage, and strategic analysis report storage.
- **Applied:** Yes (via `docker-compose exec backend python manage migrate conversations`)
