...
Add the 4 new serializers **after** the existing [`DocumentChunkSerializer`](src/backend/documents/serializers.py:99) class (line 124), maintaining the existing docstring style and field-level `help_text` convention.
...
| `test_serializes_output` | Output dict has correct types (UUID→str, etc.) |
